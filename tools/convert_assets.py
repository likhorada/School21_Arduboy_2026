...
def main():
    ...
    destination = ROOT / "src" / "assets" / "player_frames.h"
    if args.check:
        if not destination.exists() or destination.read_text(encoding="ascii") != player_text:
            raise ValueError(f"{destination}: missing or stale; rerun converter")
    elif not args.inspect:
        destination.parent.mkdir(parents=True, exist_ok=True)
        destination.write_text(player_text, encoding="ascii")
