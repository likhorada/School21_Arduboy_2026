...
Stage maps are stored as horizontal row runs with an offset every eight rows.
This keeps collision lookup bounded without paying for mostly-empty raw maps.
Stage 1 (index 0) has no walls and no map.
"""
...
def row_run_pack(mask):
    """Encode rows as [count, x, length, ...], indexed every eight rows."""
    data = bytearray()
    blocks = []
    for y in range(STAGE_H):
        if y % 8 == 0:
        ...
        x = 0
        while x < STAGE_W:
            if not mask[y * STAGE_W + x]:
                x += 1
                continue
            start = x
            while x < STAGE_W and mask[y * STAGE_W + x]:
                x += 1
            data.extend((start, x - start))
            data[count_at] += 1
    return bytes(data), tuple(blocks)
...
def stage_layouts_header(stages):
    ...
    for name, data, blocks in stages:
        symbol = name + "_rows"
        rows = ["    " + ", ".join(f"0x{value:02x}" for value in data[i:i + 16]) + ","
                for i in range(0, len(data), 16)]
        lines += [
            f"const uint8_t {symbol}[] PROGMEM = {{",
        ] + rows + ["};", f"const uint16_t {name}_blocks[] PROGMEM = {{",
                      "    " + ", ".join(str(value) for value in blocks) + ",",
                      "};"]
        lines.append("")
    return "\n".join(lines)
...
def main():
    ...
    for name, source in STAGE_SOURCES.items():
        ...
        rows, blocks = row_run_pack(mask)
        stages.append((name, rows, blocks))
        stage_bytes = len(rows) + len(blocks) * 2
        stage_total += stage_bytes
        print(f"{source}: {sum(mask)} lit px -> {stage_bytes} B row runs")
    ...
    print(f"Generated binary payload: {len(blob) + BLOB_PAD + stage_total:6d} B "
          f"(menus {len(blob) + BLOB_PAD} B, stage maps {stage_total} B)")
